mlm.mediation <- function(FUNC.xm, FIXED.xm,
  ...
  # rm(list=ls())
  # gc()
  # source('~/Dropbox/Programs/RFunctions_TKoscik/mRi/mlm.stacked.R')
  # library(MASS)
  # df <- read.csv("/home/tim/Dropbox/Analyses/wil/dualSpin_fmri_regions.csv")
  # df$Nacc.z <- scale(df$Nacc)
  # # df$response.z <- scale(df$takepass_contrast)
  # df$response.z <- (df$takepass_contrast == 1) *1
  # df$selfEV.z <- scale(df$selfEV)
  # df$otherEV.z <- scale(df$otherEV)
  # DATA <- df
  # FUNC.xm <- "lmer"
  # FIXED.xm <- "Nacc.z ~ selfEV.z"
  # FUNC.xmy <- "glmer"
  # FIXED.xmy <- "response.z ~ Nacc.z + selfEV.z"
  # RANDOM <- "(1|ptp)"
  # x.name <- "selfEV.z"
  # m.name <- "Nacc.z"
  #

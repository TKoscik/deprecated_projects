...
# Set Voxel processing order
  if (!is.null(vx.order)) {
    if (is.numeric(vx.order)) {
      for (i in 1:n.masks) {
        which.voxels[[i]] <- which.voxels[[i]][order(
          which.voxels[[i]][ , vx.order[1]],
          which.voxels[[i]][ , vx.order[2]],
          which.voxels[[i]][ , vx.order[3]],
          decreasing=as.logical(vx.order[4])), ]
      }
    } else {
      if (vx.order == "rand") {
        for (i in 1:n.masks) {
          which.voxels[[i]] <- which.voxels[[i]][sample(x = seq_len(nrow(which.voxels[[i]])),
                                                        size = nrow(which.voxels[[i]]),
                                                        replace = FALSE), ]
        }
      }
    }
